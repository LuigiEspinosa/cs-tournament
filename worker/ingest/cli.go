...
func parseAndRecord(ctx context.Context, s store.DemoStore, parser Parser, statRec db.StatRecorder, roster db.RosterReader, matchID, demoID int64, storageKey string) (parseOutcome, error) {
	...
	for _, pl := range result.Players {
		...
		rows = append(rows, db.StatRow{
			...
			// The FR-18 core seven (Story 5.1) — carried onto the row exactly like RoundsWon.
			Assists:       pl.Assists,
			...
			// The FR-20 derived three (Story 5.3) — likewise unconditional, so a player with no opening duel
			// and no clutch writes explicit zeros and the empty jsonb object `{}`, never NULLs. Clutches is a
			// MAP, so this copies a REFERENCE into the row; nothing mutates the parse result after mapping, and
			// nothing downstream may mutate row.Clutches or pl.Clutches.
