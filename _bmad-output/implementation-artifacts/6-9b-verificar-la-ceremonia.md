...
| **AC9's `stage2-resolve.json` `unreachable_outcome_kinds` marker** | shipped — ⛔ **but only after 6.9a's code review (2026-08-09) caught that it was NOT.** This row said "shipped" while 6.9a's own Completion Notes said "NOT done" and the subtask box was ticked: three artifacts, three answers, no code. The review wrote the marker (`UNREACHABLE_STAGE2_OUTCOME_KINDS = ("shared",)`). ⚠ Recorded because 6.9b is told not to close these twice — a false "shipped" here meant nobody would ever have added it. |
| ⛔⛔ **AC3's completeness test — "the browser verifier is proven to run with EVERY non-bundle read stubbed to throw"** | **NOT shipped, and it is now YOURS.** AC3 was never part of 6.9a's AC5–AC9 carve-out, so this clause belonged to 6.9a on paper — but the artifact it tests is your verifier, which did not exist. Deferred to 6.9b by Cuatro at the 6.9a review, reason: *untestable until the verifier exists*. ⭐ Fold it into **AC5's** reproduction harness: AC5 already re-derives the whole ceremony from the bundle alone, so the completeness test is that same run with every non-bundle read stubbed to throw. ⚠ `0025:8-10` is the standard — *"the JS verifier needs NOTHING outside the bundle; if it does, the bundle is incomplete (a spec bug)"* — so a failure here is a BUNDLE defect to escalate, not a stub to loosen. |
| ⛔ **`crypto.subtle` feature detection** (`deferred-work.md:289`) | **NOT shipped and correctly so** — it is unreachable until your button ships, and it becomes live the moment it does. Still your AC9. |
| ⚠ **`deferred-work.md:265-266` — zero-width / unbounded award `name`** | **RE-OPENED by 6.9a's code review.** 6.9a claimed §9.5's ASCII restriction gave this teeth; it does not — DECISION N drops `name` from the bundle, so no producer-controlled field can be non-ASCII and that refusal is unreachable in production. The guard belongs where names reach a viewer — **your verify strip and the feed** — as a reject-list for C0/C1 controls, U+200B/200C/200D/200E/200F/FEFF and the bidi overrides, plus a length bound. |
